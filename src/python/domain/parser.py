...
from domain.message import NoteOnMessage, MidoMessageHelper
...
class Parser:
    @staticmethod
    def to_mido_messages(messages: list[NoteOnMessage]):
        """
        ...
        mido_messages: list[Message] = []
        while queues.size() > 0:
            queue = queues.pop(0)

            mido_message = queue.toMidoChannelVoiceMessage(seek_time)
            mido_messages.append(mido_message)
